...
__noinit static uint8_t nvram_buf[NVRAM_BUFFER_SIZE];
...
static int region_scan(struct region_info *region)
{
	...
	if (found) {
		LOG_DBG("found region seg offset 0x%x", region->seg_offset);
		err = region_seg_scan(region);
		if (err) {
			LOG_ERR("invalid region seg offset 0x%x", region->seg_offset);
			return -1;
		}
	}
	else {
		LOG_DBG("region %s: first init", region->name);
		...
		nvram_config_load_cb cb, void *cb_arg)
{
	...
	while (offs < region->seg_size) {
		...
		addr = region->seg_offset + offs;
		/* read item header */
		region_read(region, addr, (uint8_t *)&item, sizeof(struct nvram_item));
		...
		if (item.name_size + item.data_size + 1 > NVRAM_MAX_NAME_SIZE + NVRAM_MAX_DATA_SIZE) {
			rc = -EINVAL;
			LOG_ERR("name size: %d, data size: %d", item.name_size, item.data_size);
			break;
		}

		addr += sizeof(struct nvram_item);
		/* read config name */
		region_read(region, addr, name, item.name_size);
		//printk("name: %s\n", name);
		addr += item.name_size;

		/* read config data */
		region_read(region, addr, val, item.data_size);
		//printk("value: %s\n", val);

		if ((cb != NULL) && cb(name, val, item.data_size, cb_arg)) {
			rc = -EINVAL;
			LOG_ERR("stop\n");
			break;
		}
...
void nvram_region_info_init(void)
{
	...
	nvram_part = partition_get_part(PARTITION_NAME_NVRAM_FACTORY);
	if (nvram_part) {
		factory_nvram_region.base_addr = nvram_part->offset;
		factory_nvram_region.total_size = CONFIG_NVRAM_FACTORY_REGION_SIZE;
	...
	nvram_part = partition_get_part(PARTITION_NAME_NVRAM_USER);
	if (nvram_part) {
		user_nvram_region.base_addr = nvram_part->offset;
		user_nvram_region.total_size = CONFIG_NVRAM_USER_REGION_SIZE;
...
#ifdef CONFIG_NVRAM_STORAGE_FACTORY_RW_REGION
	nvram_part = partition_get_part(PARTITION_NAME_NVRAM_FACTORY_RW);
	if (nvram_part) {
		factory_rw_nvram_region.base_addr = nvram_part->offset;
		factory_rw_nvram_region.total_size = CONFIG_NVRAM_FACTORY_RW_REGION_SIZE;
