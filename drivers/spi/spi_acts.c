...
static void spi_acts_set_clk(const struct acts_spi_config *cfg, uint32_t freq_hz)
{
...
static const struct spi_driver_api spi_acts_driver_api = {
...
#ifdef CONFIG_PM_DEVICE
static void spi_acts_active(const struct device *dev)
{
	const struct acts_spi_config *config = DEV_CFG(dev);;
...
static int spi_acts_suspend(const struct device *dev)
{
	const struct acts_spi_config *config = DEV_CFG(dev);;
	struct acts_spi_data *data = DEV_DATA(dev);

	/* spi transfer ongoing check */
	if (spi_acts_transfer_ongoing(data)) {
		return -EBUSY;
	}

	/* disable spi controller clock */
	acts_clock_peripheral_disable(config->clock_id);

	/* dsiable spi controller */
...
static int spi_acts_pm_control(const struct device *device,
				   uint32_t command,
				   void *context, device_pm_cb cb, void *arg)
{
	...
	case DEVICE_PM_ACTIVE_STATE:
		if (data->suspend_flag)
			spi_acts_active(device);
		data->suspend_flag = false;
		break;
	case DEVICE_PM_SUSPEND_STATE:
		ret = spi_acts_suspend(device);
...
#else
#define spi_acts_pm_control device_pm_control_nop
#endif
...
#define SPI_ACTS_DEFINE_CONFIG(n)					\
	static const struct acts_pin_config pins_spi##n[] =		\
		{CONFIG_SPI_##n##_MFP};	\
	static const struct acts_spi_config spi_acts_config_##n = {    \
		   ...
		   .pinmux = pins_spi##n,			   \
		   .pinmux_size = ARRAY_SIZE(pins_spi##n), \
	};
