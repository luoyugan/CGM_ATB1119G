...
#define PMUADC_WAIT_OVER_SAMPLE_US                            (1000)
...
static int pmuadc_wait_sample_complete(const struct device *dev, uint8_t ch_index)
{
	...
	while(!(ch_index & pmuadc_reg->pending)) {
		if (k_cyc_to_us_floor32(k_cycle_get_32() - timestamp)
			...
			LOG_ERR("failed to get ADC channel:%d PD:0x%x", ch_index, pmuadc_reg->pending);
			return -ETIMEDOUT;
		}
	}
			...
			const struct adc_sequence *sequence)
{
	...
	if (channels & PMUADC_PD_BATV_PD) {
		...
		if (ret)
			return ret;
		data->mdata.v.battery_voltage = pmuadc_reg->charge_data & CHARGI_DATA_CHARGI_MASK;;
	}

	if (channels & PMUADC_PD_CHARGI_PD)
		data->mdata.v.charging_current = pmuadc_reg->bat_data & BATADC_DATA_BATV_MASK;
...
static void pmuadc_isr(void *arg)
{
	...
	if (pending & PMUADC_PD_BATV_PD) {
		...
		data->mdata.v.battery_voltage = val;
		data->channels &= ~BIT(1);
	...
	if (pending & PMUADC_PD_SENSOR_PD) {
		...
		data->mdata.v.sensor_temperature = val;
		data->channels &= ~BIT(3);
...
static int pmuadc_bias_setting(const struct device *dev)
{
	...
	reg |= CONFIG_PMUADC_IBIAS_BUF_SEL;

	reg &= ~PMUADC_CTL_REG_IBIAS_ADC_MASK;
	reg |= CONFIG_PMUADC_IBIAS_ADC_SEL;
...
int adc_pm_control(const struct device *device,
			       uint32_t command,
