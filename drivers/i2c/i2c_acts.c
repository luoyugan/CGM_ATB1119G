	...
	if(I2C_FIFOSTAT_CECB & fifostat){
		if (data->msg_buf_ptr < cur_msg->len){
			...
			if(len > MAX_CNT_ONCE){
				...
				i2c->cmd = I2C_CMD_EXEC | I2C_CMD_MSS | I2C_CMD_DE; // 0x8900
			}else{
				data->xfer_len += len;
				i2c->datcnt = len;
				i2c->cmd = I2C_CMD_NS | I2C_CMD_SE|I2C_CMD_EXEC | I2C_CMD_MSS | I2C_CMD_DE; // 0x8f00;
			}
			if (data->state != STATE_READ_DATA) {
				/* write data to FIFO */
				while (!(i2c->fifostat & I2C_FIFOSTAT_TFF) &&
			     ...
			     uint8_t num_msgs, uint16_t addr)
{
	...
	ret = __i2c_acts_transfer(dev, msgs, num_msgs, addr);
	if (ret) {
...
int i2c_acts_init(const struct device *dev)
{
	...
	acts_pinmux_setup_pins(config->pinmux, config->pinmux_size);
...
static void i2c_slave_acts_isr(struct device *dev)
{
	...
	if (stat & I2C_STAT_STPD) {
		...
		i2c->ctl &= ~I2C_CTL_GRAS_ACK;
		data->status = I2C_SLAVE_STATUS_STOPED;
 	}
out:
	i2c->ctl |= I2C_CTL_RB;
}
...
#ifdef CONFIG_PM_DEVICE
static void i2c_acts_active(const struct device *dev)
{
	const struct acts_i2c_config *config = DEV_CFG(dev);;
	struct acts_i2c_data *data = DEV_DATA(dev);
	...
	acts_reset_peripheral_deassert(config->reset_id);

	acts_pinmux_setup_pins(config->pinmux, config->pinmux_size);

	i2c_acts_set_clk(config->base, data->clk_freq);

#ifdef CONFIG_I2C_SLAVE
	if (data->slave_attached) {
		struct i2c_acts_controller *i2c = config->base;
		i2c->addr = data->slave_cfg->address << 1;
		i2c->ctl = I2C_CTL_IRQE | I2C_CTL_EN | I2C_CTL_RB | I2C_CTL_GRAS_ACK | I2C_CTL_STD_IRQ_EN;
	}
#endif
}

static int i2c_acts_suspend(const struct device *dev)
{
	const struct acts_i2c_config *config = DEV_CFG(dev);;
	struct acts_i2c_data *data = DEV_DATA(dev);
...
#ifdef CONFIG_I2C_SLAVE
	if (data->slave_attached && !data->master_active) {
		if (data->status == I2C_SLAVE_STATUS_START ||
			data->status == I2C_SLAVE_STATUS_ADDRESS ||
			data->status == I2C_SLAVE_STATUS_MASTER_WRITE ||
			data->status == I2C_SLAVE_STATUS_MASTER_READ) {
			return -EBUSY;
	...
	/* enable i2c controller clock */
	acts_clock_peripheral_disable(config->clock_id);

	/* dsiable i2c controller */
	acts_reset_peripheral_assert(config->reset_id);

	for (int i = 0; i < config->pinmux_size; i++) {
		acts_pinmux_set(config->pinmux[i].pin_num, 0x1000);
	}

	return 0;
}

static int i2c_acts_pm_control(const struct device *device,
			       uint32_t command,
			       void *context, device_pm_cb cb, void *arg)
{
	...
	case DEVICE_PM_ACTIVE_STATE:
		if (data->suspend_flag)
			i2c_acts_active(device);
		data->suspend_flag = false;
		break;
	case DEVICE_PM_SUSPEND_STATE:
		ret = i2c_acts_suspend(device);
		data->suspend_flag = !ret;
		break;
...
#else
#define i2c_acts_pm_control device_pm_control_nop
#endif
...
#define I2C_ACTS_INIT(n)						\
	...
	static const struct acts_pin_config pins_i2c##n[] =		\
		{ CONFIG_I2C_##n##_MFP};	\
									\
	static const struct acts_i2c_config i2c##n##_acts_config = {	\
		...
		.irq_id = IRQ_ID_I2C##n,			\
		.pinmux = pins_i2c##n,				\
		.pinmux_size = ARRAY_SIZE(pins_i2c##n),			\
