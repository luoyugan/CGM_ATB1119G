...
static int gpio_et6416_config(struct device *dev,
			  gpio_pin_t pin,
			  gpio_flags_t flags)
{
...
static int et6416_init(struct device *dev)
{
	const struct et6416_config *cfg = dev->config;
	struct et6416_drv_data *drv_data = dev->data;
	int rc, i;
...
#ifdef CONFIG_GPIO_ET6416_INTERRUPT
static int gpio_et6416_manage_callback(struct device *dev,
...
static int gpio_et6416_enable_callback(struct device *dev,
					  gpio_pin_t pin)
{
	struct et6416_drv_data *data = dev->data;
...
static int gpio_et6416_disable_callback(struct device *dev,
					   gpio_pin_t pin)
{
	struct et6416_drv_data *data = dev->data;
...
DEVICE_AND_API_INIT(et6416, CONFIG_EXTEND_GPIO_NAME,
		    et6416_init, &et6416_drvdata, &et6416_cfg,
