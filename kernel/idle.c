...
void idle(void *p1, void *unused2, void *unused3)
{
	...
	while (true) {
		...
		/* 32k calibration */
		struct k_sem *rc32k_calib_sem = btdrv_get_rc32k_calib_sem();
		if(k_sem_take(rc32k_calib_sem, K_NO_WAIT) == 0)
			rc32k_acts_calib_event();
#endif
		 ...
		 */
		ret = z_set_timeout_expiry((ticks < IDLE_THRESH) ? 1 : ticks, true);
		if (ret)
			ticks = 1;
