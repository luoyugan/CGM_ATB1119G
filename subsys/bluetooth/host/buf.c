...
#ifdef CONFIG_BT_MIN_SIZE_BUF
#define NET_BUF_SIZE sizeof(struct net_buf)
#define CONFIG_BT_MIN_RX_BUF_COUNT ((CONFIG_BT_RX_BUF_COUNT - CONFIG_BT_SMP_RX_BUF_COUNT) * \
	(CONFIG_BT_RX_BUF_LEN + NET_BUF_SIZE) / (CONFIG_BT_MIN_RX_BUF_LEN + NET_BUF_SIZE))
NET_BUF_POOL_FIXED_DEFINE(hci_rx_pool, CONFIG_BT_SMP_RX_BUF_COUNT,
			  BT_BUF_RX_SIZE, NULL);
NET_BUF_POOL_FIXED_DEFINE(hci_min_rx_pool, CONFIG_BT_MIN_RX_BUF_COUNT,
			  CONFIG_BT_MIN_RX_BUF_LEN, NULL);
#else
...
/* actions add start */
#define L2CAP_BR_MAX_MTU_A2DP 895
...
#else
#ifdef CONFIG_BT_MIN_SIZE_BUF
	if (alloc_len > CONFIG_BT_MIN_RX_BUF_LEN) {
		buf = net_buf_alloc_len(&hci_rx_pool, alloc_len, timeout);
	} else {
		buf = net_buf_alloc_len(&hci_min_rx_pool, alloc_len, timeout);
		if (buf == NULL)
			buf = net_buf_alloc_len(&hci_rx_pool, alloc_len, timeout);
	}
#else
...
static struct net_buf *bt_buf_get_cmd_complete_len(k_timeout_t timeout, int len)
{
	struct net_buf *buf;
	unsigned int key;
			       ...
			       k_timeout_t timeout, int len)
{
	...
	case BT_HCI_EVT_CMD_COMPLETE:
	case BT_HCI_EVT_CMD_STATUS:
		return bt_buf_get_cmd_complete_len(timeout, len);
	default:
#if defined(CONFIG_BT_DISCARDABLE_BUF_COUNT)
		if (discardable) {
			struct net_buf *buf;
