...
#define CONFIG_BT_OLDEST_KEYS_MAX 6
struct bt_oldest_key {
	uint8_t id;
	uint8_t unpair;
	bt_addr_le_t addr;
};
...
uint8_t key_cnt = 0;
uint8_t bt_clear_oldest_keys_work_init = 0;
__noinit struct bt_oldest_key bt_oldest_keys[CONFIG_BT_OLDEST_KEYS_MAX];
__noinit struct k_delayed_work bt_clear_oldest_keys_work;

extern void sys_pm_reboot(int type);
static void bt_clear_oldest_keys_handler(struct k_work *work)
{
	for (int i = 0; i < CONFIG_BT_OLDEST_KEYS_MAX; i++) {
		if (bt_addr_le_cmp(&bt_oldest_keys[i].addr, BT_ADDR_LE_ANY)) {
			char key[BT_SETTINGS_KEY_MAX];

			/* Delete stored keys from flash */
			if (bt_oldest_keys[i].id) {
				char id[4];
				u8_to_dec(id, sizeof(id), bt_oldest_keys[i].id);
				bt_settings_encode_key(key, sizeof(key), "keys",
							   &bt_oldest_keys[i].addr, id);
			} else {
				bt_settings_encode_key(key, sizeof(key), "keys",
							   &bt_oldest_keys[i].addr, NULL);
			}

			BT_DBG("Deleting key %s", log_strdup(key));
			settings_delete(key);
			if (bt_oldest_keys[i].unpair) {
				bt_unpair(bt_oldest_keys[i].id, &bt_oldest_keys[i].addr);
			}
		}
	}

	/* too many keys to delate ? */
	if (key_cnt > CONFIG_BT_OLDEST_KEYS_MAX) {
		sys_pm_reboot(0);
	}
}
#endif
...
struct bt_keys *bt_keys_get_addr(uint8_t id, const bt_addr_le_t *addr)
{
	...
	if (first_free_slot == ARRAY_SIZE(key_pool)) {
		...
		if (key_cnt++ < CONFIG_BT_OLDEST_KEYS_MAX) {
			...
			if (!bt_clear_oldest_keys_work_init) {
				bt_clear_oldest_keys_work_init = 1;
				memset(bt_oldest_keys, 0, sizeof(bt_oldest_keys));
				k_delayed_work_init(&bt_clear_oldest_keys_work, &bt_clear_oldest_keys_handler);
			}
			bt_oldest_keys[key_cnt-1].id = oldest->id;
			bt_oldest_keys[key_cnt-1].unpair = 1;
			bt_addr_le_copy(&bt_oldest_keys[key_cnt-1].addr, &oldest->addr);
			keys = bt_keys_find_addr(oldest->id, &oldest->addr);
			if (keys) {
			...
			k_delayed_work_submit(&bt_clear_oldest_keys_work, K_NO_WAIT);
		}
...
int bt_keys_store(struct bt_keys *keys)
{
	...
	if (!(keys->keys & (BT_KEYS_LTK | BT_KEYS_LTK_P256)) ||
		(keys->enc_size > BT_SMP_MAX_ENC_KEY_SIZE || keys->enc_size < BT_SMP_MIN_ENC_KEY_SIZE)) {
		BT_ERR("Invalid keys for %s (%s), enc_size: %d, flags: %d, keys: %d", bt_addr_le_str(&keys->addr),
			   log_strdup(key), keys->enc_size, keys->flags, keys->keys);
		return -EINVAL;
	}
	...
	if (len != BT_KEYS_STORAGE_LEN) {
		do {
			...
			if (key_cnt++ < CONFIG_BT_OLDEST_KEYS_MAX) {
				...
				bt_oldest_keys[key_cnt-1].id = keys->id;
				bt_oldest_keys[key_cnt-1].unpair = 0;
				bt_addr_le_copy(&bt_oldest_keys[key_cnt-1].addr, &keys->addr);
				if (keys->state & BT_KEYS_ID_ADDED) {
					bt_id_del(keys);
				}
				memset(keys, 0, sizeof(*keys));
