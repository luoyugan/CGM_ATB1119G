...
#define DMA_PRINTK_BUF_SIZE  (4*1024)
...
static unsigned char dma_printk_buffer[DMA_PRINTK_BUF_SIZE];
...
static int _dma_start_tx(printk_ctx_t *ctx)
{
	...
	if (ctx->dma_setting.read_len == 0){
		...
		if(data_size > 0){
			if(data_size > (DMA_PRINTK_BUF_SIZE/4))  // Prevents printk  threads from blocking for too long
				data_size = (DMA_PRINTK_BUF_SIZE/4);
...
static void cbuf_output(const unsigned char *buf, unsigned int len, printk_ctx_t *p_ctx)
{
	uint32_t irq_flag;
	int free_space;
...
void uart_dma_send_buf(const uint8_t *buf, int len)
{
	printk_ctx_t *pctx = &g_pr_ctx;
	if(!pctx->init){
		k_str_out(buf, len);
...
static int printk_dma_init(const struct device *dev)
{
	...
	cbuf_init(&pctx->cbuf, (void *)dma_printk_buffer, DMA_PRINTK_BUF_SIZE);
