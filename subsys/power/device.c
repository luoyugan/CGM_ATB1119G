...
static device_idx_t num_susp, num_early_susp;
...
static int _pm_devices(uint32_t state)
{
	...
	for (int i = num_pm - 1; i >= 0; i--) {
		...
		if ((rc != -ENOTSUP) && (rc != 0)) {
			LOG_INF("%s did not enter %s state: %d",
...
int pm_early_suspend(void)
{
	...
	for (int i = num_pm - 1; i >= 0; i--) {
		 ...
		 * and set the device states accordingly.
		 */
		rc = device_set_power_state(dev, DEVICE_PM_EARLY_SUSPEND_STATE, NULL, NULL);
		if ((rc != -ENOTSUP) && (rc != 0)) {
			LOG_INF("did not enter early suspend state: %d", rc);
			return rc;
		}
...
{
	device_idx_t pmi = num_pm - num_early_susp;
	num_susp = 0;
	while (pmi < num_pm) {
...
{
	return _pm_devices(DEVICE_PM_OFF_STATE);
}
