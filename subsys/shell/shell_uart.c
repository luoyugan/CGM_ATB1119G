...
void uart_handle_sysrq_str(struct device * port, uint8_t *buf, int len)
{
	int i;
	for(i = 0; i < len ; i++)
...
extern void uart_dma_send_buf(const uint8_t *buf, int len);
		 ...
		 const void *data, size_t length, size_t *cnt)
{
	...
	#ifdef CONFIG_ACTIONS_PRINTK_DMA
	uart_dma_send_buf(data8, length);
	*cnt = length;
	sh_uart->ctrl_blk->handler(SHELL_TRANSPORT_EVT_TX_RDY,
				   sh_uart->ctrl_blk->context);
