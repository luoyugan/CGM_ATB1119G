...
void spicache_profile_dump(const struct shell *shell, struct spicache_profile *profile)
{
	...
	if (total != 0)
		shell_print(shell, "totoal hit: %12d    totoal miss: %12d    hit ratio: %d%%\n\n",
			...
			(profile->total_hit_cnt ) / (total / 100));
}
					...
					size_t argc, char **argv)
{
	...
	if (to_start) {
		const char *path = (argc > 3) ? argv[3] : "/SD:/.input.rec";
...
static int shell_cmd_trng(const struct shell *shell,
					size_t argc, char **argv)
{
	uint32_t low, high;

	se_trng_init();
	se_trng_process(&low, &high);
	se_trng_deinit();

	shell_print(shell, "low 0x%x, high 0x%x", low, high);
	return 0;
}					
...
#endif
	SHELL_CMD(trng, NULL, "trng", shell_cmd_trng),
