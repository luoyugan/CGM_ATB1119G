...
extern uint8_t Image$$ER_VECTOR$$Base[];
#define REMAP_VECTOR_START (uint32_t)(Image$$ER_VECTOR$$Base)

/* APP INFO */
uint8_t __irq_vector_table MAGIC[4] = {'A', 'T', 'B', 0};
uint32_t __irq_vector_table ROM_START = REMAP_VECTOR_START;
uint32_t __irq_vector_table ROM_END = 0;
uint32_t __irq_vector_table ENTRY = 0;
uint32_t __irq_vector_table NOR_ADDR = 0x2000;
uint32_t __irq_vector_table CHEKSUM_DATA = 0;
uint32_t __irq_vector_table CHEKSUM_HDR = 0;
