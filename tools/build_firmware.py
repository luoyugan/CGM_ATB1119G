...
#        print(self.partitions)
        ...
        for part in self.partitions:
            if ('file_name' in part.keys()) and ('BOOT' == part['type']):
                ...
                with open(os.path.join(self.bin_dir, self.boot_file_name), 'rb') as f:
                    f.seek(0, 0)
                    header = f.read(8)
                    h_magic0,h_magic1 = struct.unpack('II',header)
                    size = os.path.getsize(os.path.join(self.bin_dir, self.boot_file_name))
                    if(h_magic0 != 0x48544341 and h_magic1 != 0x41435448 and size > 0xcc0):
                        print('boot file length 4k')
                        self.boot_file_4k = True
                    f.close()
    ...
    def generate_crc_file(self):
        ...
        for crc_file in crc_files:
            if crc_file != self.boot_file_name or not self.boot_file_4k:
    ...
    def encrypt_files(self):
        for part in self.partitions:
            ...
            if ('true' == part['enable_encryption']):
                ...
                if part['file_name'] != self.boot_file_name or not self.boot_file_4k:
    ...
    def generate_firmware_partition_table(self):
        ...
        if boot_file != '' and not self.boot_file_4k:
