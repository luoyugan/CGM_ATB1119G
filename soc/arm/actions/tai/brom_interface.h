...
#define  RC32K_FILTER1          1
#define  RC32K_FILTER3          3
#define  RC32K_FILTER7          7
...
typedef struct 
{
	...
	void (*p_launch)(boot_type_e type, unsigned int run_addr, unsigned int storage_addr);       /* launcher º¯Êý*/
	void* p_spinor_api;	    
	void* (*p_memset)(void *dst, int val, unsigned int count);
	void* (*p_memcpy)(void *dest, const void *src, unsigned int count);
	int (*p_memcmp)(const void *s1, const void *s2, unsigned int len);                                                           /* spinor mem */
	int (*p_vsnprintf)(char* buf, int size, unsigned int linesep, const char* fmt, va_list args);                                                              /* spinor mem */
	unsigned int (*p_calculate_crc32)(unsigned char *buffer, unsigned int buf_len,	unsigned int crc_initial, bool last);	
	int (*p_verify_signature)(const unsigned char *pub_bytes, uint8_t *digest_bytes, const unsigned char *sig_bytes);
	unsigned char * (*p_image_tlv_find)(void *image_head, unsigned short it_type, unsigned short *it_len, int is_prot);
	void (*p_SHA256_hash)(unsigned char *data, unsigned int sig_check_len,  unsigned char *digest_bytes);
	unsigned int (*p_image_checksum32)(void *data, unsigned int len);
	unsigned int (*p_brom_card_write)(unsigned int,unsigned int,unsigned char*);             /* card write º¯Êý*/
}brom_api_t;
...
#define  pbrom_libc_api   ((lib_api_t *)0x0000684c)
...
#define p_brom_libgui_api ((brom_libgui_api_t *)0x00006820)
