...
/*!< Force DAC to start when the external trigger source (set from command #AOUT_CMD_SET_DAC_TRIGGER_SRC)
 * does not trigger.
 * int audio_out_control(dev, handle, #AOUT_CMD_DAC_FORCE_START, NULL)
 *
 * Returns 0 if successful and negative errno code if error.
 */
