...
__subsystem struct uart_driver_api {
	...
	int (*uart_mfp_enable)(const struct device *dev);
	int (*uart_mfp_disable)(const struct device *dev);
#endif

#ifdef CONFIG_BT_CTRL_BLE_BQB
	int (*irq_pending_clear)(const struct device *dev);
#endif
};

#ifdef CONFIG_UART_TEMP_COVER_MFP

static inline int uart_mfp_enable(const struct device *dev)
{
	const struct uart_driver_api *api =
				(const struct uart_driver_api *)dev->api;
	return api->uart_mfp_enable(dev);
}

static inline int uart_mfp_disable(const struct device *dev)
{
	const struct uart_driver_api *api =
				(const struct uart_driver_api *)dev->api;
	return api->uart_mfp_disable(dev);
}

#endif

#ifdef CONFIG_BT_CTRL_BLE_BQB
static inline int uart_irq_pending_clear(const struct device *dev)
{
	const struct uart_driver_api *api =
				(const struct uart_driver_api *)dev->api;
	return api->irq_pending_clear(dev);
}
#endif
