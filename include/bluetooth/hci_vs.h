...
#define BT_HCI_OP_VS_SET_LOCAL_FEAT          BT_OP(BT_OGF_VS, 0x3F2)  /*!< Set Local Feature opcode. */
typedef struct {
	uint8_t  val[8];
} local_feat_t;

#define BT_HCI_OP_VS_SET_OP_FLAGS						 BT_OP(BT_OGF_VS, 0x3F3)  /*!< Set Operational Flags opcode. */
struct set_op_flags {
	uint32_t flags;
	bool en;
}__packed;
